    ...
    <nav className="w-full bg-white border-b border-gray-200 z-50 sticky top-0 left-0">
      <div className="flex items-center justify-between h-[70px] px-6 md:px-[60px]">
        ...
        <ul className="hidden md:flex items-center gap-6 text-[#414c79] font-medium">
          {["HOME", "OUR COMPANY", "SERVICES", "RESOURCES", "CONTACT US"].map(
            (item) =>
              item === "SERVICES" ? (
                <li
                  key={item}
                ...
                >
                  <div className="flex items-center gap-1 hover:text-[#002e5a] transition-all">
                    {item}
                    <ChevronDown size={16} />
                  </div>
                  ...
                  <div className="absolute left-0 top-[35px] w-[250px] bg-white shadow-lg rounded-md border border-gray-100 opacity-0 group-hover:opacity-100 transition-opacity duration-200 z-50">
                    {services.map((service, i) => (
                      <div key={i} className="group/sub px-4 py-2 hover:bg-gray-50 relative">
                        <a
                          href={service.link}
                          className="flex items-center justify-between text-sm font-semibold text-[#002e5a]"
                        >
                          {service.title}
                          <ChevronDown
                            size={14}
                  ...
                  <div className="absolute left-0 bottom-[-4px] w-0 h-[2px] bg-[#fdb930] transition-all group-hover:w-full"></div>
                </li>
              ) : (
                <li key={item} className="relative group">
                  <a
                    ...
                    className="hover:text-[#002e5a] transition-all"
                  >
                    {item}
                  </a>
                  <div className="absolute left-0 bottom-[-4px] w-0 h-[2px] bg-[#fdb930] transition-all group-hover:w-full"></div>
          ...
          <button
            ...
            className="bg-[#002e5a] px-3 py-1 text-white rounded-sm transition-all duration-300 hover:bg-transparent hover:text-[#002e5a] hover:border hover:border-[#002e5a] ml-4"
          >
            GET STARTED
          </button>
        </ul>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-white border-t border-gray-200 flex flex-col items-start px-6 py-4 gap-3">
          <a href="/" className="hover:text-[#002e5a] transition-all">
            HOME
          </a>
          <a href="/about-us" className="hover:text-[#002e5a] transition-all">
            OUR COMPANY
          </a>
              ...
              <div className="pl-4 mt-2 flex flex-col gap-2">
                {services.map((service, i) => (
                  <div key={i}>
                    <a
                      href={service.link}
                      className="font-semibold text-[#002e5a] text-sm"
                    >
                      {service.title}
                    </a>
                    <ul className="pl-4 mt-1 flex flex-col gap-1">
                      {service.sub.map((subItem, j) => (
                        <li key={j}>
                          <a
                            href={subItem.link}
                            className="text-gray-700 text-sm hover:text-[#002e5a]"
                          >
                            {subItem.name}
                          </a>
                        </li>
          ...
          <a href="/resources" className="hover:text-[#002e5a] transition-all">
            RESOURCES
          </a>
          <a href="/contact" className="hover:text-[#002e5a] transition-all">
            CONTACT US
          </a>
          <button
            onClick={() => {
